...
이웃한 노드(또는 형제 노드)가 데이터를 너무 적게 가지고 있다면(즉, 점유율이 기준치 아래로 떨어지면) 형제 노드와 병합됩니다. 이것을 *언더플로우*[^underflow]라고 부릅니다. 언더플로우 시나리오는 기본적으로 2가지가 있습니다: 공통인 부모 노드를 가지는 연속된 두 개의 노드가 하나의 노드에 들어갈 만큼의 데이터를 가지고 있는 경우에는 두 노드의 데이터는 병합되어야 합니다; 
- 
