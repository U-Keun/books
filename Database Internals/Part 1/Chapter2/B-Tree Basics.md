...
B-Tree는 각각의 노드에 저장돼있는 키의 개수(fanout)로 특징지어집니다. 키의 개수(fanout)가 많으면 트리의 균형을 유지하는 데 필요한 구조적 변경 비용을 상각하고 단일 블록 또는 여러 블록에 자식 노드에 대한 키와 포인터를 저장하여 탐색 횟수를 줄이는 데 도움이 됩니다.
