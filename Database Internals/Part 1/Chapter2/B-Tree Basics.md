...
HDD와 SSD 모두 개별 바이트가 아닌 메모리 청크를 다루기 때문에(즉, 데이터 블록 단위로 액세스), 대부분의 운영체제는 *블럭 장치* 추상화가 있습니다. 이것은 내부적인 디스크 구조를 숨기고 내부적으로 I/O 작업을 버퍼링하므로, 저장 매체서 하나의 단어를 읽을 때 해당 단어가 포함된 전체 블록을 읽습니다.
