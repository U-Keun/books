...
###### 구분자 키(Separator Key)
B-Tree 노드에 저장된 키는 *인덱스 항목*[^index entry], *구분자 키*[^separator key], 또는 *분할 셀*[^divider cell]이라고 불립니다. 이것은 트리를 서브트리(*브랜치*[^branch] 또는 *서브레인지*[^subrange])로 나누고, 대응되는 키의 범위를 가지고 있습니다. 키는 이진 탐색이 가능하도록 정렬되어 있습니다. 서브트리는 
