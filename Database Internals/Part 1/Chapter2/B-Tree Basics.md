...
##### 디스크 기반 스토리지에서의 트리
위에서 말했듯, 노드의 탐색을 수행하는 데 unbalanced tree는 최악의 경우 $O(N)$의 시간복잡도를 가지고, balanced tree는 $O(\log_2 N)$의 시간복잡도를 가집니다. 동시에, balanced tree는 *fanout*[^2]도 작기 때문에 노드를 재배치하고 포인터를 업데이트해서 균형적인 상태를 유지해야 합니다. 하지만 이것은 BST의 유지 관리 비용을 높여서 디스크 기반 데이터 구조로는 비효율적입니다.
