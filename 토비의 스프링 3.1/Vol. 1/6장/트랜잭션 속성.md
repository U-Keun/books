...
###### 격리 수준
서버 환경에서는 여러 개의 트랜잭션이 동시에 진행될 수 있다. 모든 트랜잭션이 독립적이면 좋겠지만, 그러면 성능이 떨어질 수밖에 없다. 그래서 각 트랜잭션의 *격리 수준*[^isolation level]을 조정해서 가능한 한 많은 트랜잭션을 동시에 진행시키면서 문제가 발생하지 않도록 제어해야 한다. 
