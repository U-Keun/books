...
> 큐에 새 노드 $v_{r + 1}$는, 큐에서 나오는 노드 $u$의 인접 리스트에 포함된 노드이므로, $v_{r + 1}.d=u.d + 1$이고, 귀납적 가정에 의해 $u.d\le v_1.d$이므로 $v_{r + 1}.d\le v_1.d + 1$이다. 또한, 귀납적 가정에 의해 $v_r.d\le u.d + 1$이므로, $v_r.d\le u.d + 1=v_{r+1}.d$이다.
