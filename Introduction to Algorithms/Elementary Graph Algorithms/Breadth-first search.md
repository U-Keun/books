...
그래프의 모든 노드는 흰색인 상태로 탐색이 시작됩니다. 각각의 노드는 회색 또는 검정색으로 칠해질 수 있고, 탐색이 모두 끝나면 모든 노드는 검정색으로 칠해지게 됩니다. 

각각의 노드를 흰색, 회색 또는 검정색으로 칠합니다. 모든 노드는 흰색으로 시작하고, 각각은 나중에 회색 또는 검은색이 될 수 있습니다. 탐색하는 동안 조우한 노드는 방문되었다는 것을 의미하도록 흰색이 아닌 다른 색으로 지정됩니다. 회색 또는 검정색으로 칠해진 노드는 모두 방문했던 노드를 의미하지만, '너비를 우선으로' 탐색하는 방식을 위해 구분되어 있습니다. 만일 간선 $(u, v)\in E$에 대해 $u$가 검정색으로 칠해져 있다면, 노드 $v$는 반드시 회색 또는 검정색으로 칠해져 있어야 합니다. 즉, 검정색으로 칠해진 노드와 인접한 노드는 모두 한 번 이상 방문되었음을 의미합니다. 회색으로 칠해진 노드는 흰색 노드와 인접해 있을 수 있는데, 이것은 회색 노드가 방문된 노드와 그렇지 않은 노드의 경계라는 것을 의미합니다. 
