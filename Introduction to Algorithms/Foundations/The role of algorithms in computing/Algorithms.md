...
###### Parallelism
우리는 프로세서 클록 속도가 꾸준히 증가할 것이라고 예상했지만, 물리적인 한계가 있습니다. 클록 속도가 증가하면 전력 밀도가 초선형적으로 증가하기 때문에, 클록 속도가 너무 높으면 칩이 녹을 수 있습니다. 따라서 초당 더 많은 계산을 수행하기 위해 칩은 이제 하나의 프로세서가 아닌 여러 개의 "코어"를 포함하도록 설계되고 있습니다. 이런 '멀티 코어' 설계는 여러 대의 컴퓨터가 하나의 칩에 모인 것으로 볼 수 있습니다. 이것은 일종의 '병렬 컴퓨터'입니다. 멀티코어 컴퓨터에서 최상의 성능을 끌어내기 위해서는 병렬 처리를 염두에 둔 알고리즘을 설계해야 합니다. 27장에서는 여러 코어를 활용하는 '멀티스레드' 알고리즘 모델을 제시합니다. 이 모델은 챔피언십 체스 프로그램을 포함한 여러 성공적인 컴퓨터 프로그램의 기초를 형성합니다.
