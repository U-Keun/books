...
그러면 `in_interrupt()` 매크로가 계산되는 방식을 통해 현재 인터럽트 컨텍스트 안에 있음을 확인할 수 있는 것이다. 아래의 비트 연산을 생각해보자.
```C
(preempt_count() & (NMI_MASK | HARDIRQ_MASK | SOFTIRQ_MASK))
```
`preempt_count()`는 인터럽트와 관련된 비트 필드를 가지고 있고, 인터럽트와 관련된 값과 `&` 연산을 하고 있으므로, 인터럽트 컨텍스트에 있다면 위의 값은 `0`이 아닌 어떤 값이 될 것이다. 